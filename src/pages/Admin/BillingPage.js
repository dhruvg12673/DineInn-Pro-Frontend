...
const BillingPage = () => {
  ...
  const fetchActiveOrder = useCallback(async () => {
    ...
    try {
      const res = await axios.get(`${API_BASE}/api/orders/by-table`, {
      ...
      if (loadedOrder && loadedOrder.id) {
        try {
          ...
          console.error("Could not fetch tip, defaulting to 0.", tipError);
          setRetrievedTip(0);
        }
      }
      ...
      if (err.response?.status === 404) {
        ...
        alert(err.response?.data?.error || 'Could not load the active order for this table.');
        navigate(`/${restaurantId}/admin/tables`);
      }
    }
  ...
  const filteredItems = selectedCategory && menuData[selectedCategory]
   ...
   const addItemToBill = (item) => {
    ...
    const newItem = { ...item, quantity: 1, price: parseFloat(item.price) };
...
const generateBillPdf = () => {
  ...
  const handleSendKOT = async (andPrint = false) => {
    ...
    const orderData = {
      ...
      categoryid: activeOrder?.categoryid || categories.find(c => c.name === selectedCategory)?.id,
  ...
  const handleBillFormSubmit = async () => {
    ...
    try {
      ...
      // If an email is needed but the PDF wasn't printed first, generate it now.
      if (billFormData.email && !pdfToSend) {
        const { pdfBase64 } = await generateBillPdf();
        pdfToSend = pdfBase64;
      }
      
      // 1. Finalize the bill on the server
      ...
      alert(`Bill for table ${tableNumber} has been finalized!`);
      ...
      // 3. Navigate back to the tables page
      navigate(`/${restaurantId}/admin/tables`, { replace: true });
  ...
  const handleCustomItemSubmit = () => {
    ...
    const newItem = {
      ...
      price: parseFloat(customItemForm.amount),
      quantity: 1,
    };
